...
*   `notes_api/`
    *   `src/` - Основной код приложения
        ...
        *   `database.py` - Настройка подключения к БД (движок, сессия)
        *   `models.py` - ORM-модели SQLAlchemy
        *   `schemas.py` - Pydantic-схемы для валидации данных
        *   `crud.py` - Логика CRUD-операций и маршруты
    *   `tests/` - Тесты приложения
        *   `__init__.py` - Инициализация пакета `tests`
        *   `test_api.py` - Тесты API с использованием `pytest`
    *   `images/` - Изображения для README (если есть)
        *   `curl_example.png` - Пример: скриншот вывода curl
    *   `.gitignore` - Файл игнорирования для Git
    *   `README.md` - Документация проекта
    *   `requirements.txt` - Зависимости проекта (если используется)
